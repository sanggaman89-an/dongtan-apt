...
def fetch_all_data():
    ...
    lawd_cd = "41597" # 화성시 코드
    
    curr = datetime.now()
    # 최근 72개월(6년)치 데이터 수집 목록 생성
    ...
    for i, month in enumerate(months):
        status_text.text(f"🔍 동탄 전수 데이터 수집 중: {month} ({i+1}/72)")
...
# [날짜 에러 방지] 튜플/리스트 형태 모두 대응하도록 수정
...
if not df.empty:
    ...
    # [로직 2] 반등 최고가 찾기 (역사적 최고가 날짜 이후 중 최고)
