...
st.title("📊 동탄 실거래가 정밀 분석 (타입/평형 표시)")
    ...
    if not clean_df.empty:
        ...
        if not p_df.empty:
            ...
            def format_type_pyeong(area):
                py = round(area / 3.3 * 1.3) # 전용면적 기준 공급평형 대략 계산
            ...
            res['2위금액'] = res.apply(lambda x: f"{int(x['역사2위_가']):,} 만원 ({x['역사2위_일']})", axis=1)
            ...
            # 컬럼 순서 조정: 전용면적 대신 '타입(평형)' 사용
            f_cols = ['순번', '해당동', '아파트명', '타입(평형)', '층', '거래금액', '변화량', '직전거래금액', '특징', '1위대비 2위', '2위금액', '역사적 1위(최고)', '계약일자']
